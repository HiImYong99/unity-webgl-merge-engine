...
def html(loc,i,img):
    ...
    if i==1:
        sizes=[150,170,190,215,240,270,300,350,390,440,520]
        ...
        body=f"<div class='card'><img src='{b64(img)}' style='margin-top:-{crop[0]*0.878:.0f}px'></div>"
    return f"""<!doctype html><html><head><meta charset=utf-8>
<link href="https://fonts.googleapis.com/css2?family={fam[0]}&family=Nunito:wght@700;900&display=block" rel=stylesheet>
<style>*{{margin:0;box-sizing:border-box}}body{{width:1080px;height:1920px;overflow:hidden;background:linear-gradient(165deg,{c1},{c2});font-family:{fam[1]},'Nunito',sans-serif;color:#fff;text-align:center}}
.head{{height:400px;padding:80px 56px 0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:22px}}
h1{{font-size:96px;line-height:1.08;font-weight:{fam[2]};white-space:nowrap;letter-spacing:-1px;text-shadow:0 6px 0 rgba(0,0,0,.12)}}
p{{font-size:40px;font-weight:700;font-family:{SUB.get(loc,"'Nunito'")},'Nunito',sans-serif;opacity:.95;white-space:nowrap}}
.card{{width:948px;height:1440px;margin:20px auto 0;border-radius:64px;overflow:hidden;background:#fdf8f6;box-shadow:0 30px 80px rgba(0,0,0,.28),0 0 0 10px rgba(255,255,255,.35)}}
.card img{{width:948px;display:block}}
...
<script>document.fonts.ready.then(()=>{{for(const [id,min] of [['h',54],['s',26]]){{const e=document.getElementById(id);let f=parseFloat(getComputedStyle(e).fontSize);while(e.offsetWidth>968&&f>min){{f-=1;e.style.fontSize=f+'px'}}}}document.title='ready'}})</script></body></html>"""
async def main(locs):
    os.makedirs(f"{ROOT}/html",exist_ok=True)
    async with HChrome(port=9399,lang='en-US') as c:
        await c.metrics(1080,1920,1,mobile=False)
        for loc in locs:
            src=pick(loc); out=f"{ROOT}/out/{loc}"; os.makedirs(out,exist_ok=True)
