...
/// <summary>
/// 디저트 팝 프로젝트 초기 세팅 및 씬 자동 구성을 위한 에디터 유틸리티
/// </summary>
public class DessertPopSetup : EditorWindow
{
    ...
    private static void CreateGameContainer()
    {
        ...
        // 바닥 및 벽면은 CreateWall 헬퍼 사용 (생략 가능하나 구조상 유지)
    ...
    {
        // 사운드 리소스 및 버튼 클릭음 자동 연결
        SoundMgr snd = FindObjectOfType<SoundMgr>();
        if (snd == null) return;

        // 명시적으로 Resources에서 로드하여 할당 (예시)
        ...
        // UI 버튼 클릭음 일괄 연결 등 추가 가능
        Debug.Log("[DessertPopSetup] Audio assets linked (if found in Resources).");
    }

    private static void CreateDefaultData()
    {
        // DessertEvolutionData 로드 로직 (기존 유지)
        DessertEvolutionData data = AssetDatabase.LoadAssetAtPath<DessertEvolutionData>("Assets/_Project/Data/DessertEvolutionData.asset");
        if (data == null)
        {
